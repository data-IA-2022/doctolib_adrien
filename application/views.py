...
def comptes(request):
    ...
    return render(request, "comptes.html", {"regexMDP": regexMDP, "message": message})
...
def edaia(request):
    if request.user.role == "patient":
        return redirect(
            "https://media.tenor.com/2euSOQYdz8oAAAAj/this-was-technically-illegal-maclen-stanley.gif"
        )
    else:
...
def historique(request):
    ...
    if role == "responsable":
        ...
        idpatients = medecinPatient.objects.filter(
            idMedecin=Utilisateur.objects.filter(username=request.user.username)[0].id
        ).values_list("idPatient", flat=True)
        dataFormulaireSante = [
            form.values
            for idpatient in idpatients
            for form in FormulaireSante.objects.filter(patient=idpatient).values()
        ]

    elif role == "patient":
        dataFormulaireSante = [
            form.values
            for form in FormulaireSante.objects.filter(
                patient=Utilisateur.objects.filter(username=request.user.username)[0].id
            ).values()
        ]
...
def associationMedecinPatient(request):
    ...
    listePatientsNonAssocies = [
        patient for patient in patients if patient not in listePatientsAssocies
    ]
    ...
    if request.method == "POST":
        ...
        medecinPatient(
            idMedecin=Utilisateur.objects.filter(username=medecin)[0],
            idPatient=Utilisateur.objects.filter(username=patient)[0],
        ).save()
        return redirect("associationMedecinPatient")
    return render(
        request,
        "associationMedecinPatient.html",
        {
