...
def alimentationPatients():
    listePatients = pd.read_csv(join("authentification", "datas", "listePatients.csv"))
    for index, valeurs in listePatients.iterrows():
        # champDBB = Utilisateur._meta.get_fields()

        Utilisateur.objects.create_user(
            username=valeurs.username, password=valeurs.motDePasse, role="patient"
        )


def alimentationMedecin():
    listeMedecins = pd.read_csv(join("authentification", "datas", "listeMedecins.csv"))
    for index, valeurs in listeMedecins.iterrows():
        Utilisateur.objects.create_user(
            username=valeurs.username, password=valeurs.motDePasse, role="medecin"
        )
